            ...
            if (!strcmp (key, "NAME")) {
                ...
                *ncount = atoi (field);
                ...
                if (norm==CC_USER) {
                    ...
                    dat->w = CC_SAFE_MALLOC (*ncount, double);
                    if (!dat->w) {
                        CCutil_freedatagroup (dat);
                        return 1;
                    }
                    for (i = 0; i < *ncount; i++) {
                        fscanf (in, "%*d %lf %lf %lf %lf",
                               &(dat->x[i]), &(dat->y[i]), &(dat->z[i]), &(dat->w[i]));
                    }
                } else if ((norm & CC_NORM_SIZE_BITS) == CC_D2_NORM_SIZE) {
                    dat->x = CC_SAFE_MALLOC (*ncount, double);
                    if (!dat->x) {
                        CCutil_freedatagroup (dat);
                        return 1;
...
static int read_user_text (char *datname, int *ncount, CCdata_user *userdat)
{
    ...
    userdat->x = (double *) NULL;
    userdat->y = (double *) NULL;
    userdat->z = (double *) NULL;
    // userdat->w = (double *) NULL;
    ...
    userdat->x = CC_SAFE_MALLOC (*ncount, double);
    userdat->y = CC_SAFE_MALLOC (*ncount, double);
    userdat->z = CC_SAFE_MALLOC (*ncount, double);
    // userdat->w = CC_SAFE_MALLOC (*ncount, double);
    if (userdat->x == (double *) NULL ||
        userdat->y == (double *) NULL ||
        userdat->z == (double *) NULL //||
        // userdat->w == (double *) NULL
    ...
    for (i=0; i<*ncount; i++) {
        // fscanf (datin, "%lf%lf%lf%lf", &(userdat->x[i]), &(userdat->y[i]), &(userdat->z[i]), &(userdat->w[i]));
        fscanf (datin, "%lf%lf%lf", &(userdat->x[i]), &(userdat->y[i]), &(userdat->z[i]));
    }
    fclose (datin);
    datin = (FILE *) NULL;
    return 0;
 ...
 FAILURE:
    CC_IFFREE (userdat->x, double);
    CC_IFFREE (userdat->y, double);
    CC_IFFREE (userdat->z, double);
    // CC_IFFREE (userdat->w, double);
    ...
    // for (i = 0; i < *ncount; i++) {
    //     fscanf (datin, "%lf%lf%lf",
    //             &(userdat->x[i]), &(userdat->y[i]), &(userdat->z[i]));
    // }
    // fclose (datin);
    // return 0;
...
static int writemaster_user (CC_SFILE *out, int ncount, CCdata_user *userdat)
{
    ...
    for (i=0; i<ncount; i++) {
        if (CCutil_swrite_double (out, userdat->x[i]) ||
            ...
            CCutil_swrite_double (out, userdat->z[i]) //||
            // CCutil_swrite_double (out, userdat->w[i])
            ) {
            return 1;
        }
    }
...
static int readmaster_user (CC_SFILE *in, int ncount, CCdata_user *userdat)
{
    ...
    for (i=0; i<ncount; i++) {
        if (CCutil_sread_double (in, &userdat->x[i]) ||
            ...
            CCutil_sread_double (in, &userdat->z[i]) //||
            // CCutil_sread_double (in, &userdat->w[i])
            ) {
            goto FAILURE;
        }
    }
