...
static int user_edgelen (int i, int j, CCdatagroup *dat)
{
    ...
    // printf("%p", dat->userdat.x);
    // printf("hi");
    // double t1 = dat->userdat.x[i] - dat->userdat.x[j], t2 = dat->userdat.y[i] - dat->userdat.y[j], t3 = dat->userdat.z[i] - dat->userdat.z[j], t4 = dat->userdat.w[i] - dat->userdat.w[j];
    double t1 = dat->x[i] - dat->x[j], t2 = dat->y[i] - dat->y[j], t3 = dat->z[i] - dat->z[j], t4 = dat->w[i] - dat->w[j];
    // printf("t1 %f t2 %f t3 %f t4 %f", t1, t2, t3,t4);
    // double t1 = dat->x[i]-dat->x[j], t2 = dat->y[i]-dat->y[j], t3 = dat->z[i]-dat->z[j];
    // printf("p1 (%f, %f, %f) p2 (%f, %f, %f)\n", dat->userdat.x[i], dat->userdat.y[i], dat->userdat.z[i], dat->userdat.x[j], dat->userdat.y[j], dat->userdat.z[j]);
    return (int) (ceil (sqrt (t1 * t1 + t2 * t2 + t3 * t3 + t4 * t4)));
