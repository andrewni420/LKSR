...
int CClinkern_tour (int ncount, CCdatagroup *dat, int ecount,
        ...
        int *outcycle, double *val,
        int silent, double time_bound, double length_bound,
        char *saveit_name, int kicktype, CCrandstate *rstate, int *outIters, int **outtours, int numOutputs)
{
    int rval = 0;
    int i;
    int *tcyc = (int *) NULL;
    ...
    *val = cycle_length (ncount, tcyc, &D);
    ...
    rval = repeated_lin_kernighan (&G, &D, tcyc, stallcount, repeatcount,
                 ...
                 kicktype, &intptr_world, &edgelook_world, rstate, outIters, outtours, numOutputs);
    if (rval) {
...
printf ("%4d Steps   Best: %.0f   %.2f seconds (Negative %.0f) (%.0f)\n",
        ...
        if (outiter<numOutputs && round>=outIters[outiter]){
            // for (int i=0;i<ncount;i++) outtours[outiter][i]=cyc[i];
            CClinkern_flipper_cycle (&F, outtours[outiter]);
            outiter++;
        }
...
void lin_kernighan (graph *G, distobj *D, adddel *E, aqueue *Q,
